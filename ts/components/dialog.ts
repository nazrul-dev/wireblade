...
export default (options: InitOptions): DialogComponent => ({
  ...
  init () {
    window.wireblade.dispatchHook(`${options.id}:load`)
  },
  dismiss () {
    this.close()
  ...
  fillDialogIcon (icon) {
    ...
    fetch(`/wireblade/icons/${icon.style ?? 'outline'}/${icon.name}`)
  ...
  createButton (options, action) {
    fetch(`/wireblade/button?${new URLSearchParams(options as string)}`)
      .then(response => response.text())
      .then(html => {
