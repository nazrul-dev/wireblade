...
class Modal extends Component
{
    ...
    public function __construct(
        ...
        $zIndex   ??= config('wireblade.modal.zIndex');
        $maxWidth ??= config('wireblade.modal.maxWidth');
        $spacing  ??= config('wireblade.modal.spacing');
        $align    ??= config('wireblade.modal.align');

        if ($blur === null) {
            $blur = config('wireblade.modal.blur');
        }
